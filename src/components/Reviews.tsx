...
const splitArray = <T,>(array: Array<T>, numParts: number): Array<Array<T>> => {
  ...
  const baseSize = Math.floor(totalLength / numParts);
  const extraItems = totalLength % numParts;

  let startIndex = 0;

  for (let i = 0; i < numParts; i++) {
    const currentPartSize = baseSize + (i < extraItems ? 1 : 0);

    result.push(array.slice(startIndex, startIndex + currentPartSize));
...
const Review = ({ imgSrc, className, ...props }: ReviewProps) => {
  ...
  return (
    <div
      className={cn(
        `animate-fade-in w-fit rounded-[2.25rem] bg-white p-6 shadow-xl shadow-slate-900/5`,
        className,
      )}
...
const ReviewColumn = ({
  ...
  return (
    <div
      ref={columnRef}
      className={cn('m-auto animate-marquee space-y-8 py-4', className)}
